...
import {drawKeypoints, drawBoundingBox, drawSkeleton, toggleLoadingUI, TRY_RESNET_BUTTON_NAME, TRY_RESNET_BUTTON_TEXT, updateTryResNetButtonDatGuiCss} from './demo_util';
import * as partColorScales from './part_color_scales';
import Border from './tracker/Border.js';
import Persons from './tracker/Persons.js';
...
const videoElement0 = document.getElementById('video-0');
...
uploadVideo.addEventListener('change', (e) => {
  ...
  reader.onload = function(e) {
    ...
    videoElement0.src = url;

    // videoElement0.load();

    videoElement0.addEventListener('loadedmetadata', function() {
      const x = videoElement0.videoWidth;
      const y = videoElement0.videoHeight;
      ...
      // debugger;
      videoElement0.addEventListener('canplay', function() {
        context.drawImage(videoElement0, 0, 0, x, y);
        setTimeout(async () => {
          context.drawImage(videoElement0, 0, 0, x, y);
          videoElement0.style.display = 'none';
          // debugger;
          border = await Border.line(canvas, {x, y});
          videoElement0.play();
        }, 1000); // loaded
      });
    });
...
videoElement0.onplay = function() {
  const stream = videoElement0.captureStream();
...
const defaultMobileNetMultiplier = 0.5; // isMobile() ? 0.50 : 0.75;
...
window.boundingBoxColor = 'red';
const persons = new Persons();

function drawPoses(personOrPersonPartSegmentation, flipHorizontally, ctx, save) {
  ctx.beginPath();
  ...
  if (Array.isArray(personOrPersonPartSegmentation)) {
    ...
    personOrPersonPartSegmentation.forEach((personSegmentation) => {
      ...
      const rect = {x: boundingBox.minX, y: boundingBox.minY, w: boundingBox.maxX - boundingBox.minX, h: boundingBox.maxY - boundingBox.minY};
      const center = {x: Math.round(rect.x + rect.w / 2), y: Math.round(rect.y + rect.h)};

      peoples.push(center);
      // debugger;
    });

    persons.check(peoples, border);
    persons.draw(ctx);
    if (border) document.getElementById('counter').innerHTML = persons.counter.toString();

    // if (save && persons.length > 0) {
    //   if (centers.length === 10) {
    //     centers = centers.concat(centers.slice(1), persons);
    //   } else {
    //     centers.push(persons);
    //   }
    //   console.log(centers);
    // }
    ...
    // if (last > 0) {
    //   for (let i = 0; i <= centers[last].length; ++i) {
    //     if (!centers[0][i]) continue;
    //     ctx.moveTo(centers[0][i].x, centers[0][i].y);
    //     // debugger;
    //     for (let j = 1; j < centers.length; ++j) {
    //       if (!centers[j][i]) continue;
    //       ctx.lineTo(centers[j][i].x, centers[j][i].y);
    //     }
    //   }
    // }
    ctx.stroke();
 ...
 * magic happens. This function loops with a each Frame method.
 */
function segmentBodyInRealTime() {
  ...
  let prevTS = 0;
  let save = false;
  async function bodySegmentationFrame(timestamp) {
    // console.log(timestamp, prevTS)
    if ((timestamp - prevTS) > 500) {
      prevTS = timestamp;
      save = true;
    } else save = false;
      ...
      case 'segmentation':
        ...
        switch (guiState.segmentation.effect) {
          case 'mask':
            ...
            if (border) border.draw(ctx);
            drawPoses(multiPersonSegmentation, flipHorizontally, ctx, save);
            break;
          case 'bokeh':
